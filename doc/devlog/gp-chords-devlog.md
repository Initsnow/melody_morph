...
## 调性先验彻底移除（《春日影》29 小节 Esus2 回归）

用户指出《春日影.gp》29 小节：28 小节是 B7（V7/IV），29 小节第一窗的
E-B-F# 被读成 Bsus4/E（罗马 Isus4/E），并质疑"识别和弦为什么要给主音
加分"。排查确认：主音加分早在计划第 2 步就从主分数里删了，但"调内/
主音破平"（key_rank）仍留在同分排序里、且排在"低音=根音"之前——
E-B-F# 的 Esus2/Bsus4/F#sus4 三种读法分数完全相同（0.5 并列），
tonic 先验把根音拉到了 B。

修改：**彻底删除 key_rank**。调性现在完全不参与打分与破平，只决定
升/降号拼写。同分破平顺序变为
`分数 > m7/6 家族偏好 > 低音=根音 > 根音出现权重 > 模板更简 > 根音编号`。

连带影响（都是同一类同音集挂留读法，音符本身无法区分，只能靠低音/
权重说话）：

- 29 小节第一窗 Bsus4/E -> **Esus2**（用户期望的 IV 侧，罗马 IVsus2）；
- 5/6/7/8 小节 Bsus2/F# -> F#sus4（F#-B-C# 琶音，F# 最低音），
  罗马 Vsus4；67 小节同理 Esus2、82 小节 Bsus2/C# -> F#sus4/C#。
  若想在这些地方保留 Bsus2/F#（I 功能）读法，那本质上是调性/功能先验，
  与本修改的原则冲突，需要用户明确选择；
- 旧用例 F-C-E-C 由 Cadd11(no5)/F（主音 C 先验的产物）改为
  Fmaj7(no3)（低音 F = 根音），测试预期同步更新。

未引入任何上下文规则（不做"前一小节是副属和弦就给后面加分"之类）。
`uv run pytest tests` 137 passed；合成基准 30/30；样例文件《无论如何》
auto 窗口 25/37 名称、34/37 根音，无回退。

---

## 分解和弦切窗修复（《春日影》35 小节 Badd9）

用户指出 35 小节"明显的分解和弦被识别成两个和弦"：D#-B-C#-F#-C#-B
（Badd9 的分解）被切成 B(no5)/D# + Bsus2 两窗。根因是琶音合并规则里
"相对上一拍必须是跳进"——B->C# 是小二度级进，被当作经过音拒并。
但真正的音阶跑动（C-D-E-F-G）保护又不能删（有专门测试）。

修改切窗器（`segment_auto`）：

1. **跳进特征 + 内部级进可并入**：当前组已由跳进成形（分解和弦特征）
   时，组内的级进允许并入（Badd9 的 B->C#）。纯音阶跑动全程级进、
   组从未成形，仍然切散。
2. **级进并入需要后续拍接住**：级进并入必须满足"下一拍仍属于同一
   琶音"（子集/继续跳进并入），否则视为结尾经过音，不并入——避免
   40 小节把 F#-F-E 的 F 自然音并成 C#add11(no5)、73 小节把 E5->B5
   两个强力和弦并成 Esus2。
3. **同音重复不算级进**：{G#,B} 接 {G#} 是和弦延续（G#m 琶音），
   不是音阶跑动，72 小节不再被拆成 G#5 + G#m(no5)。

实测《春日影》Rhythm Guitar：35/39/77/81 -> Badd9/D#、7 -> Badd9/F#、
67 -> Esus2、72 -> G#m（分解和弦合成一窗）；40 的经过音、73 的
E5->B5 保持切分。`uv run pytest tests` 140 passed（新增 3 个切窗
回归用例）；合成基准 30/30；样例文件无回退。
