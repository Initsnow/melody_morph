...
标注粒度：freetext 写在**每个循环遍的起点**——Intro 的 2 小节循环
重复 4 遍，第 1、3、5、7 小节各标一处（同一 region 内每一遍都可见，
用户能直接看出"这遍又回到了 P1"）。标注内容是**该遍循环的完整罗马
数字**（含品质，如 ``P1: I-IV-V7-vi``）；同一 family 的不同 region、
不同遍各标各的实际进行，变体直接在谱面上可见（如 Verse 1 的遍标
``V7``、Verse 2 的遍标 ``V``），不再被 family 汇总模式抹平。
**变体遍标记**：以 family 第一次出现的那遍为参照（P1），后续遍实际
进行与它完全一致仍标 ``P1``，有变体则标 ``P1'``，谱面上直接区分
"原样重复"与"变体重复"。quality 对和声分析很重要，所以汇总模式
只用于归族/聚类，展示层按循环遍保留完整品质。
"""
